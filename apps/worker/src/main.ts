...
const worker =
  documents === undefined
    ? undefined
    : new DocumentWorker(prisma, documents, pollMs, {
        baseBackoffMs,
        maxAttempts,
        outboxLeaseMs,
      });
...
async function bootstrap(): Promise<void> {
  ...
  if (documentsEnabled) {
    if (
      ...
      throw new Error("DOCUMENT_RUNTIME_NOT_READY");
    }
