 ...
 * director. This role is deliberately separate from recommendation and
 * authority-review roles so the separation-of-duties guard remains effective.
 */
export const SYNTHETIC_ADMISSIONS_DIRECTOR_TENANT_CODE = "synthetic-school";
export const SYNTHETIC_ADMISSIONS_DIRECTOR_ROLE_KEY =
  "synthetic.admissions.director";
export const SYNTHETIC_ADMISSIONS_DIRECTOR_PERMISSIONS = Object.freeze([
...
export type SyntheticAdmissionsDirectorProvisionErrorCode =
  | "INVALID_DIRECTOR_EMAIL"
  | "DIRECTOR_EMAIL_NOT_VERIFIED"
  | "DIRECTOR_ACCOUNT_NOT_ACTIVE"
...
export class SyntheticAdmissionsDirectorProvisionError extends Error {
  constructor(
    readonly code: SyntheticAdmissionsDirectorProvisionErrorCode,
    message: string,
  ) {
    super(message);
    this.name = "SyntheticAdmissionsDirectorProvisionError";
  }
}

export interface ProvisionSyntheticAdmissionsDirectorInput {
  confirmation: string;
...
export interface ProvisionSyntheticAdmissionsDirectorResult {
  auditEventId: string;
  created: {
    auditEvent: boolean;
    membership: boolean;
  ...
  tenantCode: typeof SYNTHETIC_ADMISSIONS_DIRECTOR_TENANT_CODE;
  tenantId: string;
}
...
function normalizeDirectorEmail(value: string): string {
  ...
  if (
    ...
    throw new SyntheticAdmissionsDirectorProvisionError(
      "INVALID_DIRECTOR_EMAIL",
      "The synthetic director must use a verified @resend.dev address",
    );
  }
  return email;
}
...
export class SyntheticAdmissionsDirectorProvisioner {
  constructor(
  ...
  async provision(
    input: ProvisionSyntheticAdmissionsDirectorInput,
  ): Promise<ProvisionSyntheticAdmissionsDirectorResult> {
    if (input.stage !== "preproduction-synthetic") {
      throw new SyntheticAdmissionsDirectorProvisionError(
        "SYNTHETIC_TENANT_REQUIRED",
        "Synthetic admissions director provisioning is restricted to preproduction-synthetic",
      );
    }
    if (input.tenantCode !== SYNTHETIC_ADMISSIONS_DIRECTOR_TENANT_CODE) {
      throw new SyntheticAdmissionsDirectorProvisionError(
        "SYNTHETIC_TENANT_REQUIRED",
        "Synthetic admissions director provisioning is restricted to synthetic-school",
      );
    }
    if (input.confirmation !== SYNTHETIC_ADMISSIONS_DIRECTOR_TENANT_CODE) {
      throw new SyntheticAdmissionsDirectorProvisionError(
        "SYNTHETIC_CONFIRMATION_REQUIRED",
        "Confirmation must exactly match synthetic-school",
      );
    }
    const directorEmail = normalizeDirectorEmail(input.directorEmail);
    const tenantId = stableUuid(`admission:tenant:${input.tenantCode}`);

    return this.prisma.$transaction(async (transaction) => {
      await transaction.$queryRaw`
        SELECT pg_advisory_xact_lock(
          hashtextextended(${`admission:synthetic-admissions-director:${input.tenantCode}`}, 0)
        )::text AS lock_result
      `;
      ...
      const director = await transaction.platformUser.findUnique({
        where: { emailNormalized: directorEmail },
      });
      if (director === null) {
        throw new SyntheticAdmissionsDirectorProvisionError(
          "DIRECTOR_ACCOUNT_NOT_FOUND",
          "The director must register before synthetic provisioning",
        );
      }
      if (director.status !== "ACTIVE") {
        throw new SyntheticAdmissionsDirectorProvisionError(
          "DIRECTOR_ACCOUNT_NOT_ACTIVE",
          "The director account must be active",
        );
      }
      if (director.emailVerifiedAt === null) {
        throw new SyntheticAdmissionsDirectorProvisionError(
          "DIRECTOR_EMAIL_NOT_VERIFIED",
          "The director email must be verified",
        );
      }
      ...
      let membership = await transaction.membership.findFirst({
        where: { tenantId, userId: director.id },
      });
      if (membership === null) {
        membership = await transaction.membership.create({
          data: {
            id: stableUuid(
              `admission:synthetic-admissions-director:membership:${input.tenantCode}:${director.id}`,
            ),
            startsAt: now,
            status: "ACTIVE",
            tenantId,
        ...
        throw new SyntheticAdmissionsDirectorProvisionError(
          "SYNTHETIC_PROVISIONING_STATE_CONFLICT",
          "The director membership has incompatible state",
        );
      }

      const roleAssignmentId = stableUuid(
        `admission:synthetic-admissions-director:role:${input.tenantCode}:${director.id}`,
      );
      const existingAssignments = await transaction.roleAssignment.findMany({
        where: {
          membershipId: membership.id,
          roleKey: SYNTHETIC_ADMISSIONS_DIRECTOR_ROLE_KEY,
          tenantId,
        },
      });
      ...
      if (
        ...
        throw new SyntheticAdmissionsDirectorProvisionError(
          "SYNTHETIC_PROVISIONING_STATE_CONFLICT",
          "A non-deterministic synthetic director role already exists",
        );
      }
      ...
      if (roleAssignment === undefined) {
        roleAssignment = await transaction.roleAssignment.create({
          data: {
            id: roleAssignmentId,
            membershipId: membership.id,
            permissions: [...SYNTHETIC_ADMISSIONS_DIRECTOR_PERMISSIONS],
            roleKey: SYNTHETIC_ADMISSIONS_DIRECTOR_ROLE_KEY,
            scopes: ["*"],
            startsAt: now,
            status: "ACTIVE",
        ...
        throw new SyntheticAdmissionsDirectorProvisionError(
          "SYNTHETIC_PROVISIONING_STATE_CONFLICT",
          "The deterministic synthetic director role has incompatible state",
        );
      }

      const auditEventId = stableUuid(
        `admission:audit:synthetic-admissions-director:${input.tenantCode}:${director.id}`,
      );
      ...
      if (existingAudit === null) {
        await transaction.auditEvent.create({
          data: {
            ...
            correlationId: `synthetic-admissions-director:${input.tenantCode}`,
        ...
        existingAudit.action !== "SYNTHETIC_ADMISSIONS_DIRECTOR_PROVISIONED"
      ) {
        throw new SyntheticAdmissionsDirectorProvisionError(
          "SYNTHETIC_PROVISIONING_STATE_CONFLICT",
          "The deterministic synthetic director audit event has incompatible state",
        );
      }
